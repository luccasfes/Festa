...
function startRealTimeListener() {
    roomsRef.on('value', snapshot => {
        ...
        if (selectedRoomId && allRoomsRaw[selectedRoomId]) {
            const room = allRoomsRaw[selectedRoomId];
            renderUserList(room.presence, room.banned, selectedRoomId);
...
// Só registra os listeners depois que o DOM estiver pronto,
document.addEventListener('DOMContentLoaded', () => {
    const searchInputEl = document.getElementById('searchInput');
    const sortFilterEl = document.getElementById('sortFilter');

    if (searchInputEl) {
        searchInputEl.addEventListener('input', updateRoomsList);
    } else {
        console.warn('Elemento #searchInput não encontrado no DOM.');
    }

    if (sortFilterEl) {
        sortFilterEl.addEventListener('change', updateRoomsList);
    } else {
        console.warn('Elemento #sortFilter não encontrado no DOM.');
    }
});
...
function openDetails(id) {
    ...
    // Passando o ID e a lista de banidos também
...
function copyId() {
    ...
    // CORRIGIDO: era "currentRoomId" (variável inexistente) -> agora "selectedRoomId"
